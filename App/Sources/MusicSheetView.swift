...
struct MusicSheetView : View {
    ...
    var body: some View { 
        ...
        .frame(width: (Double(getMeasureWidth()) * Double(document.getSelectedMeasuresCount() + 3)) + Double(margin * 2),
               height: Double(measureHeight + margin) * Double(document.getSelectedInstrumentsCount()))
    }
